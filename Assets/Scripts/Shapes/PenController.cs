...
public class PenController : Singleton<PenController>
{
    ...
    public void Update()
    {
        ...
        var movement =
            transform.forward * InputManager.Devices[0].RightStick.Y * MovementSpeed * Time.deltaTime +
            transform.right * InputManager.Devices[0].LeftStick.X * MovementSpeed * Time.deltaTime +
            transform.up * InputManager.Devices[0].LeftStick.Y * MovementSpeed * Time.deltaTime;
        if (!DisableShakes)
